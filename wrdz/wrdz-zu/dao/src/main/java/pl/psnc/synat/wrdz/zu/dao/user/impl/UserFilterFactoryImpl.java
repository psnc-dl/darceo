...
/**
 * Class providing easy access to creation of filtering arguments for the queries concerning {@link User}.
 */
public class UserFilterFactoryImpl extends GenericQueryFilterFactoryImpl<User> implements UserFilterFactory {
     ...
     * 
     */
    public UserFilterFactoryImpl(CriteriaBuilder criteriaBuilder, CriteriaQuery<User> criteriaQuery, Root<User> root,
            Long epoch)
            throws IllegalArgumentException {
    ...
    @Override
    public QueryFilter<User> byUsername(String username) {
    ...
    @Override
    public QueryFilter<User> byOrganization(Long id) {
        Predicate predicate = criteriaBuilder.equal(root.get(User_.organization).get(Organization_.id), id);
        return constructQueryFilter(predicate);
    }
